...
for snr_idx, snr in enumerate(snr_values):
    ...
    for rate in range(6, 10):
        ...
        model_path = ("./saved_models/CIFAR_encoder_%.6f_snr_%.2f.pkl" % (compression_rate, snr))
        ...
        for e in range(epoch_len):
            ...
            for im, label in train_data:
                ...
                if e % 10 == 0 and counter == 1:
                    ...
                    # save the images
                    path = os.path.join('./image_recover_combination/cifar10-02/merged_images/sample-epoch-%d-compre-%.2f-snr-%.2f.png' % (
            ...
            if snr > 10 and out is not None:
                for ii in range(min(len(out), 10)):  # Save only first 10 images to avoid too many files
                    ...
                    pil_img.save(
                        "./image_recover_combination/cifar10-02/cifar_train_%d_%f_snr_%.2f.jpg" % (ii, compression_rate, snr))
