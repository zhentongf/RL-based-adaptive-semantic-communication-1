...
snr_trad_values = [random.uniform(0, 20) for _ in range(10)]
...
snr_values = []
for i in range(10):
    composite_snr = compute_composite_snr_db(
        snr_trad_db=snr_trad_values[i],
        distance_m=distance_values[i],
        rel_speed_ms=rel_speed_values[i],
        d_ref=1.0, d_min=1.0,
        n=2.0, v_ref=10.0,
        w_d=1.0, w_v=1.0,
        clip_min_db=-30.0, clip_max_db=40.0
    )
    snr_values.append(composite_snr)

print("Traditional SNR values to test:", snr_trad_values)
print("Distance values (m):", [f"{d:.1f}" for d in distance_values])
...
for snr_idx, snr in enumerate(snr_values):
    ...
    for rate in range(10):
        ...
        mlp_mnist.load_state_dict(torch.load('saved_models/MLP_MNIST.pkl'))

        # Load encoder if exists
        model_path = ('saved_models/MLP_MNIST_coder_%.6f_snr_%.2f.pkl' % (compression_rate, snr))
        ...
        for e in range(epoch_len):
            ...
            if snr < 10:
                torch.save(mlp_encoder.state_dict(), ('saved_models/MLP_MNIST_coder_%f_snr_%.2f.pkl' % (compression_rate, snr)))
            ...
            if snr < 10 and out is not None:
                for ii in range(min(len(out), 10)):  # Save only first 10 images to avoid too many files
                    ...
                    pil_img.save("image_recover_combination/mnist/mnist_train_%d_%f_snr_%.2f.jpg" % (ii, compression_rate, snr))
